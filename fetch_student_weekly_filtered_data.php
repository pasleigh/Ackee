...
if (isset($_POST['get_pm_id'])) {
   ...
   // exit();
    ...
    for($k = 0; $k < $k_max; $k++) {
        ...
        /*
        $sid_for_chart_block = array();
        for ($j = $k*$max_num_students; $j < ($k+1)*$max_num_students; $j++) {
            if($j == sizeof($all_sids)){break;}
            $sid = $all_sids[$j];
            $sid_for_chart_block[] = $sid . " " . $all_family_names[$j] . " : $j";
        }

        $sid_for_chart[] = $sid_for_chart_block;
        */
    }
    ...
    $plot_title = "Student weekly attendance filtered data for $school_title session $year_session";
