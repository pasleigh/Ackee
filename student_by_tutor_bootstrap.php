...
$display_block .= "<h1>Show marks and module boxplots for a particular student <em>by Tutor</em></H1>\n";

$display_block .= "<h1 style='color: mediumvioletred'>This page is under construction</H1>\n";
...
if (true) {
    ...
    $query = "SELECT * FROM $staff_names WHERE 1 ORDER BY familyname";
    try {
    ...
    foreach ($results as $rec) {
        ...
        $staff_username = $rec['username'];

        if ($staff_username == $this_username) {
            $selected = "selected";
        } else {
            $selected = "";
        }
        $staff_list_options .= "<option $selected value='$staff_id'>$staff_fullname</option>\n";
    }
    $display_block .= "<div class='row'>\n";
    $display_block .= "    <div class='col-sm-3'>\n";
    $display_block .= "<label for='staff_id'>Select a tutor:</label>";
    $display_block .= "<select class='form-control' name='staff_id' id='staff_id' onchange='update_student_list(this.value)'>\n";
    $display_block .= "<option selected disabled>Choose a tutor</option>\n";
    $display_block .= $staff_list_options;
    ...
    $display_block .= "<label for='pm_id'>Select a year:</label>";
    ...
    $display_block .= "<label for='student_select'>Select a tutee:</label>";
...
<head>
    <meta charset="UTF-8">
    <title>AcKee  Attendance Record Viewing: Student attendance</title>
...
<script type="text/javascript">
    ...
    function update_student_list(val)
    {
        visibility = document.getElementById("student_select_div").style.display;
        pm_id = document.getElementById("pm_id").value;
        //alert(visibility);
        if(visibility == 'block'){// update the student list
            fetch_year_select(pm_id);
        }
