...
if ($access) {
    ...
    if ($entry_exists) { // exists
        ...
        $display_block .= "<p>Setting weekly Filtered attendance data of both students and modules of $school_title $year_begin-$year_end_short, data issue ($raw_data_issue)</p>\n";
        ...
        $student_module_filtered_table = GetStudentModuleFilteredTableName($school_short_name, $year_session, $version);
        $student_module_filtered_small_table = GetStudentModuleFilteredSmallTableName($school_short_name, $year_session, $version);

        //$mod_summary_table_name = GetModuleSummaryStatsTableName($school_short_name, $year_session);
        $mod_summary_table_name_filtered = "filtered_" . GetModuleSummaryStatsTableName($school_short_name, $year_session);
        $mod_summary_table_name_filtered_small = "filtered_small_" . GetModuleSummaryStatsTableName($school_short_name, $year_session);

        // Get the table name of the raw data
        //$display_block .= "<p>Student list table: $student_list_table</p>";
        $display_block .= "<p>Student-Module table: $student_module_filtered_table</p><BR>";
        $display_block .= "<p>Module summary table: $mod_summary_table_name_filtered</p><BR>";
        $display_block .= "<p>Student-Module small table: $student_module_filtered_small_table</p><BR>";
        $display_block .= "<p>Module summary small table: $mod_summary_table_name_filtered_small</p><BR>";
        $display_block .= "<p>Step 9: Populate Filtered weekly attendance data</p>";
        ...
        for($i = 0 ; $i < $total_weeks ; $i++) {
            ...
            $query = "SELECT id, sid, module_table FROM $student_module_filtered_table WHERE 1";//sid LIKE '2010%'";
            ...
            foreach ($recs as $rec) {
                ...
                // get all teh modules for this student
                ...
                foreach ($attendance_recs as $attendance_rec) {
                    ...
                    $flagged = $attendance_rec['flagged'];
                    switch ($attend_code1) {
                        case "AA":
                            $num_AA++;
                            break;
                ...
                if( $num_total == 0 || $flagged == 1){
                    $attend_pc = -1.0;
                }else{
                    $attend_pc = round($num_present*100.0/$num_total,1);
                }
                // Write the attendance stats to the
                $query = "UPDATE $student_module_filtered_table SET w$i='$attend_pc' WHERE id='$id' ";
                // Check that the column exists
                //if ( $col_check_result = $db->query( "SHOW COLUMNS FROM $student_module_filtered_table LIKE 'w$i'" ) ) {
                ...
                foreach ($attendance_recs as $attendance_rec) {
                    $attend_code1 = $attendance_rec['attend_code1'];
                    $flagged_small = $attendance_rec['flagged_small'];
                    switch ($attend_code1) {
                        case "AA":
                            $num_AA++;
                            break;
                ...
                $num_absent = $num_AD + $num_AR + $num_UA + $num_NR;
                $num_total = $num_present+$num_absent;
                if( $num_total == 0 || $flagged_small == 0){
                    $attend_pc = -1.0;
                }else{
                    $attend_pc = round($num_present*100.0/$num_total,1);
                }
                // Write the attendance stats to the
                $query = "UPDATE $student_module_filtered_small_table SET w$i='$attend_pc' WHERE id='$id' ";
                try {
                    $results = $db->query($query);
                } catch (PDOException $ex) {
                    $this_function = __FUNCTION__;
            ...
            // Get unique list of student IDs
            $query = "SELECT * FROM $mod_summary_table_name_filtered WHERE 1";
            try {
                $results = $db->query($query);
            } catch (PDOException $ex) {
                $this_function = __FUNCTION__;
            ...
            foreach ($recs as $rec) {
                ...
                // Write the attendance stats to the
                $query = "UPDATE $mod_summary_table_name_filtered SET w$i='$attend_pc' WHERE module_code='$module_code' ";
                try {
                    $results = $db->query($query);
                } catch (PDOException $ex) {
                    $this_function = __FUNCTION__;
                ...
                // Write the attendance stats to the
                $query = "UPDATE $mod_summary_table_name_filtered_small SET w$i='$attend_pc' WHERE module_code='$module_code' ";
                try {
                    $results = $db->query($query);
                } catch (PDOException $ex) {
                    $this_function = __FUNCTION__;
