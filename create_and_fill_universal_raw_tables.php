...
if ($access) {
    ...
    if ($entry_exists) { // exists
        ...
        $display_block .= "<p>Copying raw data into a standard raw data table for $school_title $year_begin-$year_end_short, data issue ($raw_data_issue)</p>\n";
        ...
        $display_block .= "<p>Step 1: Create and fill universal raw table</p>";

        // Create the universal_raw table
        $version = 1;
        $delete = 1;
        create_universal_raw_table_structure($db, $school_short_name, $year_session, $raw_data_issue, $universal_raw_table_name, $delete);

        // loop through from the main raw table copying the data into the universal one
        if ($raw_data_version == 2) {// civil
            $query = "SELECT activity, date, start_time, end_time, sid, attend_code1, attend_code2, student_name, student_family_name FROM $raw_data_table WHERE 1";
        }else{
            $query = "SELECT activity, date, start_time, end_time, sid, attend_code1, student_first_name, student_family_name FROM $raw_data_table WHERE 1";
        }
        try {
            $results = $db->query($query);
        } catch (PDOException $ex) {
        ...
        foreach ($activity_recs as $activity_rec) {
            ...
            // get module code from activity
            $pos = strpos($activity_name, "/");
            if ($pos === FALSE) {
                $module_code = $activity_name;
            } else {
                $module_code = substr($activity_name, 0, $pos);
            }
