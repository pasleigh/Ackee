...
if (isset($_POST['get_pm_id'])) {
    ...
    if ($entry_exists) { // exists
        ...
        $school_short_name_lc = strtolower($school_short_name);
        $mod_summary_table_name = "filtered_small_summary_$school_short_name_lc" . "_$year_session";
    ...
    if ($num > 0) {
        foreach ($recs as $rec) {
            ...
            foreach ($module_recs as $module_rec) {
                ...
                $flag_low_numbers = $module_rec['flag_low_numbers'];
                $flag_low_attendance = $module_rec['flag_low_attendance'];

                if ($flag_low_numbers == 1 || $flag_low_attendance == 1) {
                    // don't include on the filtered plot
                } else {
    ...
    $plot_title = "Module summary attendance filtered data for $school_title session $year_session";
