...
if (isset($_POST['get_pm_id'])) {
    ...
    if ($level_val > 5) {
        // All
        $query = "SELECT * FROM $student_list_table WHERE 1 ORDER BY family_name ASC ";
    } else {
        $query = "SELECT * FROM $student_list_table WHERE level='$level_val'  ORDER BY family_name ASC ";
    }
    ...
    if ($num > 0) {
        foreach ($recs as $rec) {
            ...
            $student_summary_data[] = array("$sid", "$first_name", "$family_name", "$mean_attendance", "$student_level", "$email");

            // Get the modules this student attends
            $query = "SELECT * FROM $student_module_filtered_table WHERE sid=$sid";
            try {
                $results = $db->query($query);
            } catch (PDOException $ex) {
                $this_function = __FUNCTION__;
            ...
            foreach($module_recs as $module_rec){
                $modules[] = $module_rec['module_code'];
            }
    ...
    $graph_data = Array('student_summary_data' => $student_summary_data, 'student_modules' => $student_modules, 'plot_title' => $plot_title);
