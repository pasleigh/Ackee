...
// Display the dropdowns to select the module and year
    $display_block .= "<h2><em>Filtered large class</em> data. Detailed Student plots</H2>";
    $display_block .= "<div id='filter' style='visibility: hidden;'><h3>Showing data for sessions with class size >" . $low_numbers_value[intval($id)] . " and attendance > " . $low_attendance_value[intval($id)] ."% </h3></div>";
...
<script type="text/javascript">
    ...
    function update_level_list(val)
    {
        ...
        $.ajax({
            type: 'post',
            url: 'fetch_test.php',
            url: 'fetch_student_detail_filtered_data.php',
            data: {
                'get_level': level_val,
                'get_pm_id': pm_id
            },
...
*/
    function draw_checked_modules(){
        ...
        $.ajax({
            type: 'post',
            url: 'fetch_student_module_detail_filtered_data.php',
            data: {
                'get_level': g_level,
                'get_pm_id': g_pm_id,
                'get_sid': g_sid
            ...
            success: function (data) {
                ...
                // Clear the container
                $('#container').html('');
                var student_name = "<H3  class='text-center'>Flitered Large Class Attendance Records for " + g_student_data[1] + " " + g_student_data[2] + " (" + g_student_data[0] + ")</H3>";
                ...
                for(i = 0; i< g_module_summary_data.length ; i++) {
                    ...
                    if (g_module_summary_data[i][1].length > 4) {
                        ...
                        my_columnplot.title.text = "Filtered large class attendance record for module " + module_code;
